...
from models import FileListModel, FileListItem, StyleItem
from path import filesPath, filePath, fileListPath
from preferencesDialogue import Preferences
from python.widgets import ListView
...
class Ui_Form( object ):
    def setupUi( self, Form ):
        ...
        self.horizontalLayout_5 = QtGui.QHBoxLayout( self.searchWidget )
        ...
        self.widget = QtGui.QWidget( self.searchWidget )
    ...
    def retranslateUi( self, Form ):
        ...
        self.searchLineBoxInEditor.setPlaceholderText(
            QtGui.QApplication.translate( "Form", "Find", None, QtGui.QApplication.UnicodeUTF8 ) )
        ...
        self.replaceLineEdit.setPlaceholderText(
            QtGui.QApplication.translate( "Form", "Replace", None, QtGui.QApplication.UnicodeUTF8 ) )
...
class MainWidget( Ui_Form, QtGui.QWidget ):
    ...
    def initialize( self ):
        ...
        self.quitSearchWordShortcut = QtGui.QShortcut( self.searchWidgetInEditor )
        ...
        self.fileListModel.modelReset.connect( self.setCurrentIndexOfListView )
        self.prevWordButton.setEnabled( False )
        self.nextWordButton.setEnabled( False )
    ...
    def setCurrentIndexOfListView( self ):
        index = self.fileListModel.getIndex( self.fileListModel.currentFileItem )
        if index != -1:
            index = self.fileListModel.index( index )
            if index.isValid():
    ...
    def loadFile( self, index ):
        ...
        if self.fileListModel.currentFileItem != data.filename:
            ...
            self.editor.document().contentsChanged.connect( self.contentChanged )
            file.close()
            self.fileListModel.currentFileItem = data.filename


    def saveFile( self ):
        if self.fileListModel.currentFileItem:
            file = open( self.fileListModel.currentFileItem, 'w' )
    ...
    def deleteItemFromFileList( self ):
        ...
        if data:
            ...
            self.titleNameChanged( None, None, None )
            self.fileListModel.currentFileItem = None
            index = self.fileListModel.index( 0 )
            if index.isValid():
                self.loadFile( index )
            else:
